...
if(isset($_REQUEST['action']) && $_REQUEST['action']!=""  ) // after form has been submitted
{
   ...
   if(isset($_REQUEST['ids']))
   {
		...
		{
		    $query=" UPDATE blog SET status='p' WHERE id IN ($ids) ";
		...
		{
			$query=" UPDATE blog SET status='u' WHERE id IN ($ids) ";
		...
		{
			$query=" DELETE FROM blog WHERE id IN ($ids) ";
			$stmt = $conn->prepare($query);
            $stmt->execute();
		}
  ...
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
     <section class="content-header">
        <h1 >
            <span style="font-size: 15px;"><a href=""> Home </a> > Blog List</span>
      </h1>
      
         <ol class="breadcrumb">
            <li> 
                 <button style="margin-left:20px;margin-right: 30px;margin-top:-10px;" class="btn btn-info btn-icon" 
                     name="save" id="save" type="button" onclick="location.href='blog_upload.php'" >Add New</button>
            </li>
        </ol> 
    </section>
              ...
              <table id="example1" class="table table-bordered table-striped" data-page-length='50'>
                ...
                ?>
                                    <tr id="<?php echo $arr['id']; ?>">
                                        ...
                                        <td><img src="images/blog/<?php echo $arr['image_name']; ?>" width="75" height="100" ></td>  
                                        <td><?php echo $arr['blog_description']; ?></td>
                                    ...
                                    -->    
                                        <td><?php 
                                            if(strlen($arr['updated_on']) > 0 ){
                                                    ...
                                                    echo date("j M  Y", strtotime($arr['updated_on'])); 
                                                    
                                            }else{
                                                
                                                    echo date("j M  Y", strtotime($arr['dated_on'])); 
                                        ...
                                        <td> <input type="checkbox" onchange="updatestatus('<?php echo $arr['id']; ?>','<?php echo $arr['status']; ?>');" id="<?php echo $arr['id']; ?>" <?php if($arr['status']=='p') { echo "checked"; }?> data-plugin="switchery" data-color="#00b19d"/></td>
                                      <!--  <td> <input type="checkbox" onchange="updatetownstatus('<?php echo $arr['id']; ?>','<?php echo $arr['town_status']; ?>');" id="<?php echo $arr['id']; ?>" <?php if($arr['town_status']=='p') { echo "checked"; }?> data-plugin="switchery" data-color="#00b19d"/></td>
                                        -->
                                      
                                     <!--   <td> <input type="checkbox" onchange="updatefeature('<?php echo $arr['id']; ?>','<?php echo $arr['feature']; ?>');" id="<?php echo $arr['id']; ?>" <?php if($arr['feature']=='p') { echo "checked"; }?> data-plugin="switchery" data-color="#00b19d"/></td>-->
                                    ...
                                    <td>   <button type="button" class="btn btn-success" data-toggle="dropdown" aria-expanded="false" onclick="location.href='blog_upload.php?ref=<?php echo $arr['id']; ?>'">Descp/Edit<span class="m-l-5"></span></button>
    ...
    function updatestatus(elm,status) // fetching states 
{
  ...
  $.ajax({
    url: "ajax/update_status_photoshoot_pages.php?id="+elm+"&status="+status+"&page=blog",
